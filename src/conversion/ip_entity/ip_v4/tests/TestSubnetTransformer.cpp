...
auto testGetSubnetSizeFromMaskLength = [](uint8_t maskLength, uint32_t expectedValue){
    auto producedValue = SubnetTransformerIPv4().getSubnetSizeFromMaskLength(maskLength);
    ASSERT_EQ(producedValue, expectedValue);
};

auto testGetMaskLengthFromSubnetSize = [](uint32_t subnetSize, uint8_t expectedMaskLength){
    auto producedLength = SubnetTransformerIPv4().getMaskLengthFromSubnetSize(subnetSize);
